local font1 = dxCreateFont("fonts/font.ttf", 10)
local font_10 = dxCreateFont("fonts/font.ttf", 10)
local font_11 = dxCreateFont("fonts/font.ttf", 9)
...
function dxGrid:SetItemText (itemID, column, new_text )
    ...
    if __checkParams ( "SetItemText", "nns", itemID, column, new_text ) then
    ...
    function ( )
        ...
        if #dxGrid.items > 0 then
            ...
            for index, data in ipairs ( dxGrid.items ) do
                ...
                if data.vis then
                    -- Cor de baixo do fundo                
                    --dxDrawRectangle (data.x, data.y, data.w, data.h, tocolor ( 0, 0, 0, 100), data.pg );

                    -- Cor de cima
                 --   dxDrawRectangle ( data.x, data.y, data.w, 30 % data.h, tocolor (32, 31, 32, 255), data.pg );
                    ...
                    if data.se and data.se <= data.r and data.se >= seeFrom and data.se <= seeTo then
                        ...
                        -- Selecionar
                        dxDrawRectangle ( data.x, ( 30 % data.h ) + y2, data.w, 25, tocolor ( 65, 105, 255 ), data.pg );
...
function dxGrid1:SetItemText (itemID, column, new_text )
...
function dxGrid1:SetVisible ( visible )
    -- bool Gridlist:SetVisible ( bool state )
...
function dxGrid1:IsVisible ( )
    -- bool Gridlist:IsVisible()

    return self.vis
end

function dxGrid1:AddColumn ( title, width )
    -- int Gridlist:AddColumn ( string title, int width )
...
function dxGrid1:RemoveColumn ( columnIndex )
    -- bool Gridlist:RemoveColumn ( int columnIndex )
...
function dxGrid1:GetItemCount ( columnID )
    -- int Gridlist:GetItemCount ( int columnIndex )
...
function dxGrid1:GetSelectedItem ( )
    -- int Gridlist:GetSelectedItem ()

    return self.se;
end

function dxGrid1:SetSelectedItem ( itemID )
    -- bool Gridlist:SetSelectedItem ( int itemIndex )
...
function dxGrid1:GetItemDetails ( column, itemID )
    -- string, mixed Gridlist:GetItemDetails ( int columnIndex, int itemIndex )
